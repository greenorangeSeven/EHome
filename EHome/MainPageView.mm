        ...
        [[AFOSCClient sharedClient]getPath:url parameters:Nil
                                   success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                       @try {
                                           ...
                                           bannerView = [[SGFocusImageFrame alloc] initWithFrame:CGRectMake(0, 0, 320, 120) delegate:self imageItems:itemArray isAuto:NO];
                                           [bannerView scrollToIndex:0];
                                           [self.advIv addSubview:bannerView];
                                       }
                                       @catch (NSException *exception) {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    {
        if ([[usermodel getUserValueForKey:@"comm_name"] isEqualToString:@""] == NO)
        {
            self.xiaoquLb.text = [NSString stringWithFormat:@"欢迎%@业主", [usermodel getUserValueForKey:@"comm_name"]];
        }
    }
}
...
- (IBAction)ggzyAction:(id)sender {
    RechargeView *rechView = [[RechargeView alloc] init];
    rechView.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:rechView animated:YES];
}
...
- (IBAction)wyfwAction:(id)sender {
    ...
    RepairsFormView *repairsView = [[RepairsFormView alloc] init];
    repairsView.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:repairsView animated:YES];
}

- (IBAction)wytzAction:(id)sender {
    NewsTableView *noticeView = [[NewsTableView alloc] init];
    noticeView.catalog = 1;
